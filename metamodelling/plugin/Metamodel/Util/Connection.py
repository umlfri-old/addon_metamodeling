...
class Connection(CacheableObject):
    ...
    def __init__(self, obj, source, destination, points = None):
        ...
        self.labels = dict((id, ConLabelInfo(self, value[0], value[1])) for id, value in enumerate(self.object.GetType().GetLabels()))
        self.selpoint = None
...
#        CSelectableObject.__init__(self)
    ...
    def GetSquare(self, canvas, includeLabels=False):
        '''get absolute position of minimal rectangle to which fits connection
    ...
    def Paint(self, canvas, delta = (0, 0)):
        ...
        self.ValidatePoints(canvas)
        self.object.Paint(DrawingContext(canvas, self, delta))
